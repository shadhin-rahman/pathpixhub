...
export default function AccountCreditsPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="flex items-center gap-3 text-3xl sm:text-4xl font-bold tracking-tight gradient-text leading-[1.15]">
          <Star className="w-8 h-8 text-[rgb(var(--accent-text))]" />
          Path credits
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b-2 border-[rgb(var(--fg-rgb)/10%)]">
                    <th className="text-left py-4 px-3 font-bold text-[rgb(var(--fg-rgb)/70%)]"></th>
                    <th className="py-4 px-3 font-bold text-[rgb(var(--fg-rgb)/90%)]">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.label} className="border-b border-[rgb(var(--fg-rgb)/5%)]">
                      <td className="py-3.5 px-3 text-[rgb(var(--fg-rgb)/70%)]">{row.label}</td>
                      {row.values.map((val, i) => {
                        ...
                        if (val === false) {
                          return <td key={i} className="py-3.5 px-3 text-center text-[rgb(var(--fg-rgb)/20%)] font-bold">—</td>;
                        }
                        return <td key={i} className="py-3.5 px-3 text-center font-bold text-[rgb(var(--fg-rgb)/80%)]">{val}</td>;
                      })}
                    </tr>
                  ))}
                </tbody>
        ...
        {/* Sidebar — FAQs + link to full credits page */}
        <div className="space-y-6">
          ...
          <Link
            href="/credits"
            className="flex items-center justify-between p-5 rounded-2xl glass-card hover:border-[rgb(var(--accent-500)/30%)] transition-all group"
          >
            <div className="flex items-center gap-3">
              <Coins className="w-5 h-5 text-[rgb(var(--accent-text))]" />
              <div>
                <p className="font-bold text-sm">View all credit packs</p>
                <p className="text-xs text-[rgb(var(--fg-rgb)/50%)]">See full pricing & bundles</p>
              </div>
            </div>
