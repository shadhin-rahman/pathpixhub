              ...
              <ul className="mt-10 space-y-2.5">
                {TRUST_POINTS.map((t) => (
                  <li key={t} className="flex items-start gap-2.5 text-sm" style={{ color: NAVY_MUTED_2 }}>
                    <svg className="w-4 h-4 mt-0.5 shrink-0" fill="none" stroke={ACCENT} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                    {t}
                  </li>
                ))}
              </ul>
                ...
                <div className="space-y-4 text-sm">
                  ...
                  {hasAmount && (
                    <div className="flex justify-between gap-4">
                      ...
                      <span className="font-bold" style={{ color: ACCENT }}>{formatMoney(converted)} {curr.code}</span>
                    </div>
                  )}
                  {hasAmount && (
