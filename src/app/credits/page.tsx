    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 items-center gap-16">
            <div className="text-center lg:text-left max-w-2xl lg:max-w-none">
              <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[rgb(var(--accent-500)/10%)] text-[rgb(var(--accent-text))] text-sm font-bold mb-8 border border-[rgb(var(--accent-500)/15%)]">
              ...
              <p className="mt-6 text-xl text-[rgb(var(--fg-rgb)/60%)] leading-relaxed max-w-2xl lg:mx-0 mx-auto">
              ...
              <div className="mt-10 flex flex-col sm:flex-row items-center lg:justify-start justify-center gap-4">
                <a href="#bundles"
                  className="inline-flex items-center gap-2 px-10 py-4 rounded-full bg-[rgb(var(--accent-500))] text-[rgb(var(--accent-contrast))] font-bold hover:bg-[rgb(var(--accent-400))] hover:scale-[1.02] transition-all text-base">
                  Buy credits
                </a>
                <a href="#faq"
                  className="px-10 py-4 rounded-full glass-card border border-[rgb(var(--fg-rgb)/10%)] text-[rgb(var(--fg-rgb))] font-bold hover:border-[rgb(var(--accent-500)/50%)] hover:text-[rgb(var(--accent-text))] transition-all text-base">
                  How it works
                </a>
              </div>
            </div>
            <div className="relative hidden lg:block justify-self-center">
              <motion.div
                ...
                className="relative rounded-3xl border-2 border-[rgb(var(--accent-500)/25%)] bg-[var(--bg-alt)] p-4 shadow-[0_30px_80px_rgb(var(--accent-500)/20%)]"
              >
                <Image
                  ...
                  width={1376}
                  height={768}
                  className="w-[560px] h-auto rounded-2xl object-cover"
                />
                <div className="mt-4 flex items-center justify-center gap-2 text-sm font-bold text-[rgb(var(--accent-text))]">
          ...
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={b.title}
                ...
                className="rounded-2xl border-2 border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg-alt)] p-8 flex items-start gap-5"
              >
                <span className="text-3xl">{b.icon}</span>
                <div>
                  <h3 className="font-bold text-lg text-[rgb(var(--fg-rgb))]">{b.title}</h3>
      ...
      <section id="bundles" className="py-20 lg:py-24 bg-[var(--bg-alt)]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <p className="mt-4 text-lg text-[rgb(var(--fg-rgb)/60%)]">Pick a bigger pack and we&apos;ll top you up with bonus credits. <span className="font-bold text-[rgb(var(--fg-rgb))]">1 credit = $1</span> toward your orders.</p>
          </div>
      ...
      <section className="py-20 lg:py-28 bg-[var(--bg-alt)]">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="rounded-3xl bg-[var(--bg)] p-10 md:p-14 space-y-12">
            {[
              ...
              >
                <span className={`shrink-0 w-14 h-14 rounded-full ${step.color} flex items-center justify-center text-xl font-extrabold`}>
                  {step.num}
                </span>
                <div>
                  <h3 className="text-xl font-bold text-[rgb(var(--fg-rgb))]">{step.title}</h3>
                  <p className="mt-2 text-base text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{step.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* FAQ */}
      <section id="faq" className="py-20 lg:py-28 bg-[var(--bg-alt)]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-14">
            ...
            <p className="mt-4 text-lg text-[rgb(var(--fg-rgb)/55%)]">Everything you need to know about buying and using PathPixHub credits.</p>
          </div>
            ...
            {FAQS.map((f, i) => (
              <div key={i} className="rounded-2xl border-2 border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg)] overflow-hidden">
                <button type="button" onClick={() => setOpen(open === i ? null : i)}
                  ...
                  <span className="font-bold text-base text-[rgb(var(--fg-rgb))]">{f.q}</span>
                  <motion.span animate={{ rotate: open === i ? 45 : 0 }} transition={{ duration: 0.2 }}
                    className="shrink-0 w-8 h-8 rounded-full glass-card flex items-center justify-center text-[rgb(var(--fg-rgb))]">
                ...
                <motion.div initial={false}
                  ...
                  <p className="px-7 pb-6 text-base text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{f.a}</p>
                </motion.div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 lg:py-24 bg-[var(--bg)]">
        <div className="max-w-5xl mx-auto px-6">
          <div className="rounded-3xl border-2 border-dashed border-[rgb(var(--accent-500)/40%)] bg-[var(--bg-alt)] p-12 lg:p-16">
            <div className="text-center">
              <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[rgb(var(--accent-500)/10%)] text-[rgb(var(--accent-text))] text-sm font-bold mb-6 border border-[rgb(var(--accent-500)/20%)]">
                🤝 No prepayment
              </span>
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight gradient-text">
                Want to pay per order instead?
              </h2>
              <p className="mt-5 text-lg text-[rgb(var(--fg-rgb)/55%)] leading-relaxed max-w-2xl mx-auto">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={f.title}
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.08 }}
                  className="rounded-2xl border border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg)] p-8 text-center"
                >
                  <span className="text-4xl">{f.icon}</span>
                  <h3 className="mt-4 text-lg font-bold text-[rgb(var(--fg-rgb))]">{f.title}</h3>
                  <p className="mt-2 text-sm text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{f.desc}</p>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/contact"
                className="inline-flex items-center gap-2 px-10 py-4 rounded-full bg-[rgb(var(--accent-500))] text-[rgb(var(--accent-contrast))] font-bold hover:bg-[rgb(var(--accent-400))] hover:scale-[1.02] transition-all text-base shadow-lg shadow-[rgb(var(--accent-500)/15%)]">
                Order My Edits
              </Link>
              <Link href="/enterprise"
                className="px-10 py-4 rounded-full glass-card border border-[rgb(var(--fg-rgb)/10%)] text-[rgb(var(--fg-rgb))] font-bold hover:border-purple-500/50 hover:text-purple-400 transition-all text-base">
