...
export default function CreditsPage() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 items-center gap-20">
            <div className="text-center lg:text-left max-w-2xl lg:max-w-none">
              <span className="inline-flex items-center gap-2.5 px-6 py-3 rounded-full bg-[rgb(var(--accent-500)/10%)] text-[rgb(var(--accent-text))] text-sm font-bold mb-10 border border-[rgb(var(--accent-500)/15%)]">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                PathPixHub Credits
              </span>
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight gradient-text leading-[1.05]">
                Save time &amp; money with credits
              </h1>
              <p className="mt-7 text-2xl text-[rgb(var(--fg-rgb)/60%)] leading-relaxed max-w-2xl lg:mx-0 mx-auto">
              ...
              <div className="mt-12 flex flex-col sm:flex-row items-center lg:justify-start justify-center gap-5">
                <a href="#bundles"
                  className="inline-flex items-center gap-2 px-12 py-5 rounded-full bg-[rgb(var(--accent-500))] text-[rgb(var(--accent-contrast))] font-bold hover:bg-[rgb(var(--accent-400))] hover:scale-[1.02] transition-all text-lg">
                  Buy credits
                </a>
                <a href="#faq"
                  className="px-12 py-5 rounded-full glass-card border border-[rgb(var(--fg-rgb)/10%)] text-[rgb(var(--fg-rgb))] font-bold hover:border-[rgb(var(--accent-500)/50%)] hover:text-[rgb(var(--accent-text))] transition-all text-lg">
                  How it works
                </a>
              </div>
            </div>
            <div className="relative hidden lg:block justify-self-center">
              <motion.div
                ...
                className="relative rounded-3xl border-2 border-[rgb(var(--accent-500)/25%)] bg-[var(--bg-alt)] p-5 shadow-[0_30px_80px_rgb(var(--accent-500)/20%)]"
              >
                <Image
                  ...
                  width={1376}
                  height={768}
                  className="w-[600px] h-auto rounded-2xl object-cover"
                />
                <div className="mt-4 flex items-center justify-center gap-2.5 text-base font-bold text-[rgb(var(--accent-text))]">
          ...
          <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-7">
            {[
              ...
              <motion.div key={b.title}
                ...
                className="rounded-2xl border-2 border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg-alt)] p-8 lg:p-10 flex items-start gap-6"
              >
                <span className="text-4xl">{b.icon}</span>
                <div>
                  <h3 className="font-bold text-xl text-[rgb(var(--fg-rgb))]">{b.title}</h3>
                  <p className="mt-2.5 text-base text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{b.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* Bundles */}
      <section id="bundles" className="py-24 lg:py-32 bg-[var(--bg-alt)]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <span className="text-xs uppercase tracking-[0.3em] font-bold text-[rgb(var(--fg-rgb)/40%)]">Credit bundles</span>
            <h2 className="mt-4 text-3xl md:text-5xl font-bold tracking-tight gradient-text">Buy more, get more</h2>
            <p className="mt-5 text-xl text-[rgb(var(--fg-rgb)/60%)]">Pick a bigger pack and we&apos;ll top you up with bonus credits. <span className="font-bold text-[rgb(var(--fg-rgb))]">1 credit = $1</span> toward your orders.</p>
          </div>
      ...
      <section className="py-24 lg:py-32 bg-[var(--bg)]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-14">
            <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[rgb(var(--accent-500)/10%)] text-[rgb(var(--accent-text))] text-sm font-bold mb-5 border border-[rgb(var(--accent-500)/15%)]">
              Quick Plan Comparison
            </span>
      ...
      <section className="py-24 lg:py-32 bg-[var(--bg-alt)]">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="mt-4 text-3xl md:text-5xl font-bold tracking-tight gradient-text">Credits in 3 simple steps</h2>
          </div>
          <div className="rounded-3xl bg-[var(--bg)] p-12 md:p-16 space-y-14">
            {[
              ...
              >
                <span className={`shrink-0 w-16 h-16 rounded-full ${step.color} flex items-center justify-center text-2xl font-extrabold`}>
                  {step.num}
                </span>
                <div>
                  <h3 className="text-2xl font-bold text-[rgb(var(--fg-rgb))]">{step.title}</h3>
                  <p className="mt-2.5 text-lg text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{step.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* FAQ */}
      <section id="faq" className="py-24 lg:py-32 bg-[var(--bg-alt)]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-xs uppercase tracking-[0.3em] font-bold text-[rgb(var(--fg-rgb)/40%)]">FAQ</span>
            <h2 className="mt-4 text-3xl md:text-5xl font-bold tracking-tight gradient-text">Questions about credits?</h2>
            <p className="mt-5 text-xl text-[rgb(var(--fg-rgb)/55%)]">Everything you need to know about buying and using PathPixHub credits.</p>
          </div>
          <div className="space-y-5">
            {FAQS.map((f, i) => (
              <div key={i} className="rounded-2xl border-2 border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg)] overflow-hidden">
                <button type="button" onClick={() => setOpen(open === i ? null : i)}
                  className="w-full flex items-center justify-between gap-5 px-8 py-7 text-left">
                  <span className="font-bold text-lg text-[rgb(var(--fg-rgb))]">{f.q}</span>
                  <motion.span animate={{ rotate: open === i ? 45 : 0 }} transition={{ duration: 0.2 }}
                    className="shrink-0 w-10 h-10 rounded-full glass-card flex items-center justify-center text-[rgb(var(--fg-rgb))]">
                ...
                <motion.div initial={false}
                  ...
                  <p className="px-8 pb-7 text-lg text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{f.a}</p>
                </motion.div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 lg:py-32 bg-[var(--bg)]">
        <div className="max-w-5xl mx-auto px-6">
          <div className="rounded-3xl border-2 border-dashed border-[rgb(var(--accent-500)/40%)] bg-[var(--bg-alt)] p-14 lg:p-20">
            <div className="text-center">
              <span className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[rgb(var(--accent-500)/10%)] text-[rgb(var(--accent-text))] text-sm font-bold mb-7 border border-[rgb(var(--accent-500)/20%)]">
                🤝 No prepayment
              </span>
              <h2 className="text-3xl md:text-5xl font-bold tracking-tight gradient-text">
                Want to pay per order instead?
              </h2>
              <p className="mt-6 text-xl text-[rgb(var(--fg-rgb)/55%)] leading-relaxed max-w-2xl mx-auto">
            ...
            <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-7">
              {[
                ...
                <motion.div key={f.title}
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.08 }}
                  className="rounded-2xl border border-[rgb(var(--fg-rgb)/8%)] bg-[var(--bg)] p-9 text-center"
                >
                  <span className="text-5xl">{f.icon}</span>
                  <h3 className="mt-5 text-xl font-bold text-[rgb(var(--fg-rgb))]">{f.title}</h3>
                  <p className="mt-3 text-base text-[rgb(var(--fg-rgb)/55%)] leading-relaxed">{f.desc}</p>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-5">
              <Link href="/contact"
                className="inline-flex items-center gap-2 px-12 py-5 rounded-full bg-[rgb(var(--accent-500))] text-[rgb(var(--accent-contrast))] font-bold hover:bg-[rgb(var(--accent-400))] hover:scale-[1.02] transition-all text-lg shadow-lg shadow-[rgb(var(--accent-500)/15%)]">
                Order My Edits
              </Link>
              <Link href="/enterprise"
                className="px-12 py-5 rounded-full glass-card border border-[rgb(var(--fg-rgb)/10%)] text-[rgb(var(--fg-rgb))] font-bold hover:border-purple-500/50 hover:text-purple-400 transition-all text-lg">
