      ...
      <div className="flex justify-between items-center">
        ...
        >
          <span className={`ml-4 md:ml-10 flex items-center gap-3 ${isLegalPage ? "text-white" : "text-[rgb(var(--fg-rgb)/85%)]"}`}>
            <Image src="/images/logo-new.png" alt="PathPixHub" width={72} height={72} className="w-[72px] h-[72px] rounded-full" />
            <span className="text-xl md:text-2xl font-bold tracking-tight hidden sm:inline">Path<span className="text-[rgb(var(--accent-text))]">Pix</span>Hub</span>
          </span>
        </Link>
            ...
            >
              ...
              >
                <Link
                  href="/"
                  onClick={closeMenu}
                  className="block font-bold text-3xl md:text-5xl text-[rgb(var(--fg-rgb))] hover:text-[rgb(var(--accent-text))] transition-colors tracking-tighter"
                >
                  <motion.span
                    whileHover={{ x: 10 }}
                ...
                <div>
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-4 font-bold text-3xl md:text-5xl text-[rgb(var(--fg-rgb))] hover:text-[rgb(var(--accent-text))] transition-colors tracking-tighter"
                  >
                    <motion.span
                      whileHover={{ x: 10 }}
                      transition={{ type: "spring", stiffness: 200, damping: 10 }}
                      ...
                      >
                        <div className="pt-5 pb-2 pl-5 md:pl-6 space-y-5 md:space-y-6 border-l-2 border-[rgb(var(--accent-500)/25%)] ml-1 max-h-[45vh] md:max-h-[40vh] overflow-y-auto">
                          {services.map((s, i) => (
                            ...
                            >
                              <Link
                                href={`/services/${s.id}`}
                                onClick={closeMenu}
                                className="block font-bold text-xl md:text-3xl text-[rgb(var(--fg-rgb)/75%)] hover:text-[rgb(var(--accent-text))] transition-colors tracking-tighter"
                              >
                                <motion.span
                                  whileHover={{ x: 10 }}
                                  transition={{ type: "spring", stiffness: 200, damping: 10 }}
                          ...
                          <Link
                            href="/services"
                            onClick={closeMenu}
                            className="inline-flex items-center gap-2 pt-1 font-bold text-base md:text-lg text-[rgb(var(--accent-text))] hover:text-[rgb(var(--accent-text))] transition-colors tracking-tighter"
                          >
                            <motion.span
                              whileHover={{ x: 10 }}
                              transition={{ type: "spring", stiffness: 200, damping: 10 }}
              ...
              <div className="mt-12 pt-8 border-t border-[rgb(var(--fg-rgb)/10%)] space-y-6 shrink-0">
                ...
                <a href="mailto:info@pathpixhub.com" className="text-[rgb(var(--fg-rgb))] hover:text-[rgb(var(--accent-text))] transition-colors block text-xl font-medium">
                  info@pathpixhub.com
                </a>
                <div className="flex items-center gap-3">
                  <a
                    ...
                    className="w-11 h-11 rounded-full glass-card flex items-center justify-center text-[rgb(var(--fg-rgb)/70%)] hover:text-[rgb(var(--accent-text))] transition-colors"
                  >
