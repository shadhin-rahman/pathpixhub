      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-[rgb(var(--fg-rgb)/60%)] max-w-xl mx-auto">
            Select the services you need and roughly how many images, and see an estimated cost
            right away — no waiting on a reply.
          </p>
        </div>

        {/* Tab switch: pick services vs. general inquiry */}
        <div className="flex items-center gap-1 p-1 rounded-2xl glass-card border border-[rgb(var(--fg-rgb)/8%)] w-fit mx-auto mb-12">
          {[
            { key: "services" as const, label: "I know what I need" },
            { key: "general" as const, label: "Not sure yet / General" },
          ].map((t) => (
            <button
              key={t.key}
        ...
        <AnimatePresence mode="wait">
          {tab === "general" ? (
            <motion.div
              ...
              className="max-w-xl mx-auto text-center glass-card rounded-[2rem] p-10 border-[rgb(var(--fg-rgb)/10%)]"
            >
              ...
              <h4 className="mt-6 text-xl font-bold text-[rgb(var(--fg-rgb))]">Not sure which service fits?</h4>
              <p className="mt-3 text-sm text-[rgb(var(--fg-rgb)/60%)] leading-relaxed">
                No problem — tell us a bit about your project and images, and we&apos;ll recommend the
                right service and a fair price. No commitment, just a conversation.
              </p>
              <Link
                href="/contact"
                className="mt-8 inline-flex items-center justify-center px-8 py-4 rounded-full bg-[rgb(var(--accent-500))] text-[rgb(var(--accent-contrast))] font-bold hover:bg-[rgb(var(--accent-400))] transition-all text-sm"
              >
